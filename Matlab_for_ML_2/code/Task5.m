...
try
  ...
  for t=1:niter,
    ...
    for i=1:k
      ...
      %s2(:,:,i) = diag((x'-repmat(mu(:,i),1,n)).^2*Z(:,i)./sum(Z(:,i)))+(x-repmat(mu(:,i),1,n))*(x-repmat(mu(:,i),1,n))'+ diag(ones(D,1)*1);; 
      
      % To fit general Gaussians use the line:
      
      s2(:,:,i) =(x'-repmat(mu(:,i),1,n))*(repmat(Z(:,i),1,D).*(x'-repmat(mu(:,i),1,n))')./sum(Z(:,i))+ diag(ones(D,1)*1);
